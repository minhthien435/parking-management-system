...
const t = {
  vi: {
    ...
    changeStatusLabel: "Chuyển trạng thái sang",
    reopenSlotOpt: "Trống (Hoạt động)",
    reopenSlotsOpt: "Trống (Hoạt động)",
    maintenanceOpt: "Đang bảo trì",
    ...
    deleteZoneConfirmTitle: "Xác nhận xóa phân khu",
    deleteZoneConfirmDesc: "Hành động này sẽ xóa vĩnh viễn phân khu và toàn bộ ô đỗ thuộc phân khu này. Bạn chắc chắn muốn tiếp tục?",
    deleteSlotConfirmTitle: "Xác nhận xóa ô đỗ",
    deleteSlotConfirmDesc: "Bạn có chắc chắn muốn xóa ô đỗ này không?",
    deleteBulkSlotsConfirmTitle: "Xác nhận xóa hàng loạt ô đỗ",
    deleteBulkSlotsConfirmDesc: "Bạn có chắc chắn muốn xóa {count} ô đỗ đã chọn không?",
    cannotDeleteSlotsWarning: "Không thể xóa vì một số ô đỗ đang có xe, đã đặt trước hoặc đang bảo trì. Chỉ có thể xóa các ô đỗ trống (AVAILABLE).",
    deleteSelectedSlotsBtn: "Xóa các ô đỗ đã chọn",
    allZonesLabel: "Tất cả phân khu",
    editSlotTitle: "Cập nhật thuộc tính ô đỗ",
    isHandicapLabel: "Dành cho người khuyết tật",
    isElectricChargingLabel: "Hỗ trợ sạc xe điện",
  ...
  en: {
    ...
    statusLabel: "Status",
    deleteZoneConfirmTitle: "Confirm Zone Deletion",
    deleteZoneConfirmDesc: "This action will permanently delete the zone and all its slots. Are you sure you want to proceed?",
    deleteSlotConfirmTitle: "Confirm Slot Deletion",
    deleteSlotConfirmDesc: "Are you sure you want to delete this parking slot?",
    deleteBulkSlotsConfirmTitle: "Confirm Bulk Slot Deletion",
    deleteBulkSlotsConfirmDesc: "Are you sure you want to delete the {count} selected parking slots?",
    cannotDeleteSlotsWarning: "Cannot delete because some selected slots are occupied, reserved, or under maintenance. Only AVAILABLE slots can be deleted.",
    deleteSelectedSlotsBtn: "Delete Selected Slots",
    allZonesLabel: "All Zones",
    editSlotTitle: "Edit Slot Attributes",
    isHandicapLabel: "Handicap Parking Slot",
    isElectricChargingLabel: "Electric Vehicle Charging",
...
// ─── Zone Header Card ────────────────────────────────────────────────────────
function ZoneHeaderCard({ zone, onEdit, onDelete }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 shadow-sm space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        {/* Manager Actions (only on specific zones, not aggregated all zones) */}
        {!isAggregate && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        {/* Available */}
        <div className="p-3 bg-emerald-100/80 dark:bg-emerald-955/20 rounded-lg text-center flex flex-col justify-center items-center">
...
function FloorOverviewCard({ floorData, onClick, language }) {
  ...
  // Occupancy rate = (occupied + booked) / capacity
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-extrabold px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-lg group-hover:bg-blue-50 dark:group-hover:bg-blue-950/20 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
      ...
      <div className="grid grid-cols-2 gap-2 text-center">
        <div className="col-span-2 p-2 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800 rounded-lg">
          ...
          <span className="text-sm font-extrabold text-slate-855 dark:text-white">{capacity}</span>
        </div>
...
export default function ManagerSlots() {
  ...
  const [isDeleteBulkSlotsOpen, setIsDeleteBulkSlotsOpen] = useState(false);
  ...
  // Pagination (Bypassed to always show all slots)
  const currentPage = 1;
  const pageSize = 1000;
  ...
  const bulkDeleteCheck = useMemo(() => {
    if (!isBulkMode || selectedSlotIds.length === 0) {
      return { allowed: true, nonAvailableSlots: [] };
    }
    const selectedSlots = selectedSlotIds.map(id => mappedSlots.find(s => s.slot_id === id)).filter(Boolean);
    const nonAvailableSlots = selectedSlots.filter(s => s.status !== "AVAILABLE");
    return {
      allowed: nonAvailableSlots.length === 0,
      nonAvailableSlots
    };
  }, [isBulkMode, selectedSlotIds, mappedSlots]);
  ...
  const handleCreateSubmit = async (e) => {
    ...
    if (!createForm.zoneName.trim()) {
      toast.error(language === "en" ? "Zone Letter is required" : "Vui lòng chọn ký tự phân khu");
      return;
    }
  ...
  const openEditModal = async (zone) => {
    try {
      ...
      if (response.data && response.data.success && Array.isArray(response.data.data)) {
        // Support both snake_case and camelCase keys in API responses
        const found = response.data.data.find(z => (z.zone_id ?? z.zoneId) === zone.zoneId);
        if (found) {
          setEditForm({
            zoneId: found.zone_id ?? found.zoneId,
            zoneName: found.zone_name ?? found.zoneName,
            capacity: (found.capacity ?? zone.capacity).toString(),
            vehicleTypeId: (found.vehicle_type_id ?? found.vehicleTypeId).toString(),
      ...
      console.error("Error fetching latest zone details:", err);
    }

    let cleanName = zone.zoneName || "";

    setEditForm({
      zoneId: zone.zoneId,
      zoneName: cleanName,
      capacity: zone.capacity.toString(),
      vehicleTypeId: (zone.vehicleTypeName?.toLowerCase().includes("car") || zone.vehicleTypeId === 2 ? "2" : "1"),
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        toast.success(language === "en" ? "Zone updated successfully" : "Cập nhật phân khu thành công");
      ...
      toast.error(error.response?.data?.message || (language === "en" ? "Failed to update zone" : "Không thể cập nhật phân khu"));
    } finally {
      setFormSubmitting(false);
    }
  };
  ...
  const handleDeleteSlotSubmit = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || (language === "en" ? "Failed to delete slot" : "Không thể xóa ô đỗ"));
    } finally {
      setFormSubmitting(false);
    }
  };
  ...
  const handleBulkDeleteClick = () => {
    setIsDeleteBulkSlotsOpen(true);
  };

  const handleBulkDeleteSlotsSubmit = async () => {
    if (selectedSlotIds.length === 0) return;
    setFormSubmitting(true);
    const toastId = toast.loading(language === "en" ? `Deleting ${selectedSlotIds.length} slots...` : `Đang xóa ${selectedSlotIds.length} ô đỗ...`);
    try {
      await api.delete("/parking/slots/bulk", {
        data: { slot_ids: selectedSlotIds }
      });
      toast.success(language === "en" ? `Successfully deleted ${selectedSlotIds.length} slots.` : `Xóa thành công ${selectedSlotIds.length} ô đỗ.`, { id: toastId });
      setIsDeleteBulkSlotsOpen(false);
      setSelectedSlotIds([]);
      setActiveSlot(null);
      await Promise.all([fetchSlots(), fetchZoneStats()]);
    } catch (error) {
      console.error("Bulk delete slots error:", error);
      toast.error(error.response?.data?.message || (language === "en" ? "Failed to delete slots" : "Không thể xóa các ô đỗ"), { id: toastId });
    } finally {
      setFormSubmitting(false);
    }
  };
  ...
  const handleEditSlotSubmit = async (e) => {
    ...
    setFormSubmitting(true);
    try {
      const response = await api.put(`/parking/slots/${editSlotForm.slotId}/edit`, {
        is_handicap: editSlotForm.isHandicap,
        is_electric_charging: editSlotForm.isElectricCharging,
      });

      if (response.data && response.data.success) {
        toast.success(language === "en" ? "Slot attributes updated successfully" : "Cập nhật thuộc tính ô đỗ thành công");
      ...
      toast.error(error.response?.data?.message || (language === "en" ? "Failed to update slot attributes" : "Không thể cập nhật thuộc tính ô đỗ"));
    } finally {
      setFormSubmitting(false);
    }
  };
  ...
  return (
    <div className="w-full h-auto pb-12 space-y-4 font-sans antialiased text-slate-700 dark:text-slate-200">
      {selectedFloorForDetails === null ? (
        <>
          {/* ── OVERVIEW TOP HEADER WITH CREATE ZONE BUTTON ── */}
        ...
        <>
          ...
          <div className="flex flex-wrap items-center gap-2 bg-white dark:bg-slate-900 p-3 md:p-4 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="px-3 py-1.5 bg-slate-100 dark:bg-slate-850 text-sm font-bold text-slate-800 dark:text-slate-200 rounded-lg border border-slate-200 dark:border-slate-700 flex items-center gap-1.5">
              <Layers size={13} className="text-blue-500" />
              <span>{language === 'en' ? `Floor ${selectedFloorForDetails}` : `Tầng ${selectedFloorForDetails}`}</span>
            </div>

            {/* Zone and Vehicle filters removed as per request to keep it independent and simplified per floor */}
          ...
          <div className={`flex flex-col gap-5 transition-all duration-300 ${(activeSlot || isBulkMode) ? "lg:grid lg:grid-cols-2" : ""}`}>
            ...
            {/* RIGHT PANEL: SINGLE SLOT UPDATE & DELETE FOR MANAGER */}
                ...
                <form onSubmit={handleUpdateSlotStatus} className="flex flex-col flex-1 gap-4">
                  ...
                  <div className="p-3 bg-blue-50/60 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 rounded-lg flex justify-between items-center">
                    ...
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-lg border ${activeSlot.status === "AVAILABLE"
                      ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-955/20 dark:text-emerald-400 dark:border-emerald-900/50"
                      : activeSlot.status === "MAINTENANCE"
                        ...
                        : activeSlot.status === "OCCUPIED"
                          ? "bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-955/20 dark:text-rose-400 dark:border-rose-900/50"
                  ...
                  {/* Actions with extra DELETE SLOT & EDIT SLOT button for Manager */}
                  <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <button
                      ...
                      className="px-2.5 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-1 border border-slate-200 dark:border-slate-700"
                      title={t[language].editSlotTitle}
                    >
                      <Edit size={13} />
                      <span>{language === 'en' ? 'Edit Slot' : 'Sửa ô đỗ'}</span>
                    </button>
                    <button
                      type="button"
                      onClick={handleCancelSelection}
                      className="px-3 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-655 dark:text-slate-300 rounded-lg font-bold text-xs transition-all"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-300 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-600 text-white py-2.5 rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-1.5 shadow-sm cursor-pointer disabled:cursor-not-allowed"
                    >
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-5 shadow-sm flex flex-col min-h-[480px] animate-in fade-in slide-in-from-right-4 duration-200">
                ...
                <form onSubmit={handleBulkUpdateSlotStatus} className="flex flex-col flex-1 gap-4">
                  ...
                  <div className="flex gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                    <button
                      type="button"
                      disabled={!bulkDeleteCheck.allowed}
                      onClick={handleBulkDeleteClick}
                      className="px-2.5 py-2.5 bg-red-50 hover:bg-red-100 dark:bg-red-955/20 text-red-655 dark:text-red-400 rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-1.5 border border-red-200 dark:border-red-900/50 shadow-sm cursor-pointer shrink-0 disabled:bg-slate-100 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-600 disabled:border-slate-200 dark:disabled:border-slate-700 disabled:cursor-not-allowed"
                      title={t[language].deleteSelectedSlotsBtn}
                    >
                      <Trash2 size={13} />
                      <span>{language === 'en' ? 'Delete' : 'Xóa'}</span>
                    </button>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="editIsHandicap"
                    checked={editSlotForm.isHandicap}
                ...
                {/* Electric Charging checkbox */}
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="editIsElectricCharging"
                    checked={editSlotForm.isElectricCharging}
